...
class OrchestrationManager:
    ...
    def initialize_agent_with_profile(self, profile_name: str, engine_name: str = None):
        ...
        # Apply actions if defined in profile
        if profile.actions:
            agent.actions = self.nexus.get_actions(profile.actions)

        # Apply knowledge store if defined
        if profile.knowledge and agent.supports_knowledge:
            agent.knowledge_store = profile.knowledge[0] if profile.knowledge else "None"

        # Apply memory store if defined
        if profile.memory and agent.supports_memory:
            agent.memory_store = profile.memory[0] if profile.memory else "None"
...
User request: {user_input}"""
    ...
    async def _handle_delegation(
        ...
        if not match:
            return "Delegation parsing error"
